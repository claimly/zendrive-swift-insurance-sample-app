...
struct SlimLogglyConfig {
    ...
    // Over-ride the following as per requirement
    static var logglyUrlString = "https://logs-01.loggly.com/bulk/logglyKey/tag/appName"

    // Number of log entries in buffer before posting entries to Loggly. Entries will also be
    // posted when the user
    // exits the app.
    static var maxEntriesInBuffer = 100

    // Loglevel for the Loggly destination. Can be set to another level during runtime
    // This is over-rided from Lumberjack settings
    static var logglyLogLevel = LogLevel.warn
...
class SlimLogglyDestination: LogDestination {
    ...
    private func toJson(dictionary: NSDictionary) -> NSData? {
        ...
        do {
            ...
            traceMessage(msg:
                String.init(format: "ERROR: Unable to serialize json, error: %@", [error]))
            return nil
        }
    }
    ...
    private func endBackgroundTask() {
        if self.backgroundTaskIdentifier != UIBackgroundTaskInvalid {
            ...
            traceMessage(msg: "[SlimLogglyDestination] :Ending background task")
        }
    }
